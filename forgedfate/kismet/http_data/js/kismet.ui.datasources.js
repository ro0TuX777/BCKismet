...
function update_datasource2(data) {
    ...
    // Add manual source addition interface
    var manual_source = $('#ds_manual_source', ds_state['ds_content']);
    if (manual_source.length == 0) {
        manual_source = $('<div>', {
            id: 'ds_manual_source',
            class: 'accordion ui-accordion',
        })
        .append(
            $('<h3>', {
                style: 'cursor: pointer; background: #f0f0f0; padding: 10px; margin: 0; border: 1px solid #ccc;'
            })
            .html('<i class="fa fa-plus"></i> Add Custom Source <i class="fa fa-chevron-down" style="float: right;"></i>')
            .on('click', function() {
                var content = $(this).next('.k-ds-manual-form');
        ...
        .append(
            $('<div>', {
                class: 'k-ds-manual-form',
                style: 'padding: 15px; display: none; border: 1px solid #ccc; border-top: none;'
            })
            .append(
                $('<label>')
                .html('Source Definition:')
            ...
            .append(
                $('<input>', {
                    ...
                    placeholder: 'e.g., gpsnmea:/dev/ttyACM0:baud=9600:name=GPS-UBlox',
            ...
            .append(
                $('<button>', {
                    ...
                    class: 'k-ds-manual-button',
                    style: 'background: #4CAF50; color: white; padding: 8px 16px; border: none; border-radius: 4px; cursor: pointer;'
                })
                .html('<i class="fa fa-plus"></i> Add Source')
                .on('click', function() {
                    var definition = $('#manual-source-input').val().trim();
                    if (definition === '') {
                        alert('Please enter a source definition');
                        return;
                    }
                    ...
                    .done(function(response) {
                        $('#manual-source-input').val('');
                        alert('Source added successfully! Check the data sources list below.');
                    ...
                    .fail(function(xhr, status, error) {
                        var errorMsg = 'Failed to add source';
                        try {
                    ...
                    .always(function() {
                        $('#manual-add-button').prop('disabled', false).html('<i class="fa fa-plus"></i> Add Source');
                    });
                })
            )
        );

        ds_state['ds_content'].append(manual_source);
    }
