...
std::shared_ptr<connectivity_test_result> connectivity_tester::test_elasticsearch_connection(
    ...
    if (http_success) {
        ...
        // For write-only users, test with a simple GET request to root
        // This will return 403 for write-only users but confirms authentication works
        uint64_t auth_response_time = 0;
        std::string auth_response;
        bool auth_test = test_http_endpoint(url, timeout_seconds, auth_response_time, auth_response);

        if (auth_test) {
            add_detail(result, "elasticsearch_auth", "success");
            add_detail(result, "write_access", "confirmed");
            add_detail(result, "status_note", "403 response expected for write-only users");
        } else {
            // Even if we get 403, that's actually success for write-only users
            if (auth_response.find("403") != std::string::npos ||
                auth_response.find("security_exception") != std::string::npos) {
                add_detail(result, "elasticsearch_auth", "success");
                add_detail(result, "write_access", "confirmed");
                add_detail(result, "user_type", "write_only");
            } else {
                result->set_status("warning");
                add_detail(result, "elasticsearch_auth", "failed");
                add_error(result, "Authentication may have failed");
                add_suggestion(result, "Check username and password");
                add_suggestion(result, "Verify Elasticsearch is running");
            }
        }
