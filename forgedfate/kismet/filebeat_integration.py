...
class FilebeatKismetIntegrator:
    ...
    def find_kismet_logs(self):
        ...
        log_patterns = {
            ...
            'kismetdb': 'Kismet-*.kismet'  # Also look for database files
        }

        # Check multiple possible locations for Kismet logs
        search_dirs = [
            self.kismet_log_dir,  # /opt/kismet/logs
            os.getcwd(),          # Current working directory
            '.',                  # Current directory
            '/tmp'                # Sometimes logs are in temp
        ]
        ...
        for log_type, pattern in log_patterns.items():
            for search_dir in search_dirs:
                ...
                if matches:
                    ...
                    break  # Found logs in this directory, no need to check others

        return found_logs
    
    def generate_filebeat_config(self, elasticsearch_config, device_name="dragonos-laptop"):
    ...
    def setup_filebeat_integration(self, elasticsearch_config, device_name="dragonos-laptop"):
...
def main():
    ...
    parser.add_argument('--device-name', default='dragonos-laptop',
